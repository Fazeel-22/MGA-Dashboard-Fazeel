...
with left:
    ...
    if k < len(po):
        ...
        proceed_disabled = (dirc in ("↑","↓") and src not in st.session_state.next_point)
        if st.button("➡️ Proceed to Step", key=f"next_{k}", disabled=proceed_disabled):
            if on_proceed():
                st.rerun()
...
with right:
    ...
    fig = go.Figure([go.Bar(x=list(interp.keys()), y=list(interp.values()), marker_color="#007398")])
    fig.update_layout(
        title="Convex Interpolated Feasible Point",
        xaxis_title="Energy Source", yaxis_title="Capacity (GW)", height=400
    )
